...
class ReturnedProductGridService
{
    /**
     * گرید برگشت کالا — یک تراکنش در هر سطر + جمع ماه شمسی.
     *
    ...
    public static function gridForMonth(int $atelierId, ?int $jalaliYear = null, ?int $jalaliMonth = null): array
    {
        ...
        $items = ReturnedProduct::query()
            ->forAtelier($atelierId)
            ->with(['product:id,name,barcode,atelier_id'])
        ...
        foreach ($items as $item) {
            ...
            $monthTotalSale += (float) $row['sale_price'];
            $monthTotalPurchase += (float) $row['purchase_price'];
            ...
            $dailyBuckets[$dateKey]['total_sale_price'] += (float) $row['sale_price'];
            $dailyBuckets[$dateKey]['total_purchase_price'] += (float) $row['purchase_price'];
        }
     ...
     * @return array<string, mixed>
     */
    public static function formatTransactionRow(ReturnedProduct $item): array
    {
        ...
        if ($purchasePrice <= 0 && $product) {
            $purchasePrice = (float) $product->purchase_price;
        }

        return [
            'id' => (int) $item->id,
            'atelier_id' => $item->atelier_id ? (int) $item->atelier_id : null,
            'purchase_id' => $item->purchase_id ? (int) $item->purchase_id : null,
            ...
            'credit_refunded' => (float) ($item->credit_refunded ?? 0),
            'credit_earned_reversed' => (float) ($item->credit_earned_reversed ?? 0),
