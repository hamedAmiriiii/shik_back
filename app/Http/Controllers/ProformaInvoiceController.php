...
class ProformaInvoiceController extends Controller
{
    ...
    public function sell(Request $request, ProformaInvoice $proformaInvoice)
    {
        $atelierId = $this->shopAtelierIdOrAbort($request);
        if ((int) $proformaInvoice->atelier_id !== (int) $atelierId) {
            return response(['message' => 'این پیش‌فاکتور متعلق به فروشگاه شما نیست.'], 403);
        ...
        $products = [];
        foreach ($proformaInvoice->items ?? [] as $row) {
            if (! is_array($row)) {
                continue;
            }
            $line = [
            ...
            foreach (['product_id', 'produced_good_id', 'raw_material_id', 'size', 'color'] as $key) {
                if (! empty($row[$key])) {
                    $line[$key] = $row[$key];
                }
            }
        ...
        if ($products === []) {
            return response(['message' => 'پیش‌فاکتور قلمی ندارد.'], 422);
        }

        $saleRequest = Request::create('/api/purchased-products', 'POST', [
            'products' => $products,
            'payment_type' => 'cash',
            'discount_amount' => (float) $proformaInvoice->discount_amount,
            'phone' => $proformaInvoice->phone,
        ]);
        $saleRequest->headers->set('Authorization', (string) $request->header('Authorization'));
        $saleRequest->headers->set('Accept', 'application/json');
        $saleRequest->setUserResolver(fn () => $request->user());

        $response = app(PurchasedProductController::class)->store($saleRequest);
        ...
        return response([
            'message' => 'پیش‌فاکتور به‌صورت فروش ثبت شد',
